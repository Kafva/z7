...
test "Ring buffer read" {
    ...
    try std.testing.expectEqual(.{11}, try rbuf.read_offset_end_fixed(0,1));
    try std.testing.expectEqual(.{10}, try rbuf.read_offset_end_fixed(1,1));
    try std.testing.expectEqual(.{9,10}, try rbuf.read_offset_end_fixed(2,2));
    try std.testing.expectEqual(.{8}, try rbuf.read_offset_end_fixed(3,1));
    try std.testing.expectEqual(.{7,8,9}, try rbuf.read_offset_end_fixed(4,3));
}
...
test "Ring buffer OOB read" {
    ...
    try std.testing.expectError(
         ...
         rbuf.read_offset_end_fixed(0,1)
    );
    ...
    try std.testing.expectError(
         ...
         rbuf.read_offset_end_fixed(2,1)
    );

    try std.testing.expectError(
         RingBufferError.InvalidOffsetRead,
        rbuf.read_offset_end_fixed(1,3)
    );
    ...
    try std.testing.expectError(
         RingBufferError.InvalidOffsetRead,
        rbuf.read_offset_end_fixed(111, 1)
    );
}
